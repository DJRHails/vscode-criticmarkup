 ...
 * colour: the old side on the theme's own removed-text red, the new side on its
 * inserted-text green — the colours the diff editor two tabs over is already using. That is the
 * one thing a TextMate grammar cannot do, and why none of this is a grammar: a grammar can only
 * name scopes like `criticmarkup.addition`, which no theme has ever heard of, so the markup
 * comes out unstyled in every theme but the handful that opted in.
 *
 * The one grammar this extension does ship (`syntaxes/substitution-tildes.injection.json`)
 * styles nothing. It exists to take a substitution's `{~~` and `~~}` away from markdown's
 * strikethrough rule, which would otherwise read `{~~old~>new~~}` as one `~~…~~` run and strike
 * the new side — the text the suggestion is asking for — through.
 *
